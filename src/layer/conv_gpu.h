...
public:
    Conv_gpu(int channel_in, int height_in, int width_in, int channel_out,
         ...
         int pad_h = 0, int n_streams = 1, int kernel = 1) : dim_in(channel_in * height_in * width_in),
                          channel_in(channel_in), height_in(height_in), width_in(width_in),
                          channel_out(channel_out), height_kernel(height_kernel),
                          width_kernel(width_kernel), stride(stride), pad_w(pad_w), pad_h(pad_h), n_streams(n_streams),kernel(kernel)
    {
        init();
    }
