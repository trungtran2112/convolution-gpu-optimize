...
private:
    ...
    int width_out;
    int channel_out;
    cuda_kernel::kernel_type kernel;
    // weight and bias
...
public:
    ...
    Conv_gpu(int channel_in, int height_in, int width_in, int channel_out,
         ...
         int pad_h = 0, cuda_kernel::kernel_type kernel = cuda_kernel::naive) : dim_in(channel_in * height_in * width_in),
                          channel_in(channel_in), height_in(height_in), width_in(width_in),
                          channel_out(channel_out), height_kernel(height_kernel),
                          width_kernel(width_kernel), stride(stride), pad_w(pad_w), pad_h(pad_h),kernel(kernel)
    {
        init();
    }
