...
void conv_forward_cpu(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
  ...
  */
  ...
  // calculates the index into the one-dimensional array that corresponds to the 4D coordinate (i3, i2, i1, i0)
  #define y4d(i3, i2, i1, i0) y[(i3) * (M * H_out * W_out) + (i2) * (H_out * W_out) + (i1) * (W_out) + i0] // y4d(b, m, h, w)

  #define x4d(i3, i2, i1, i0) x[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0] // x4d(b, c, h + p, w + q)
  #define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0] // k4d(m, c, p, q)
    ...
    for (int m = 0; m < M; m++){// for each output feature maps
      for (int h = 0; h < H_out; h++){ // for each output element
        for (int w = 0; w < W_out; w++){
          y4d(b, m, h, w) = 0; // initialize output to 0
