...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    // const int m = blockIdx.z; // Output Channel
...
//     if (b < B){ // each image in the batch
...
__host__ void GPUInterface::conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    int inputArrayLength = B*C*H*W;
    int outputArrayLength = B*M*H_out*W_out;
    int kernelArrayLength = M*C*K*K;

    cudaMalloc((void**) &device_x, inputArrayLength*sizeof(float));
    cudaMalloc((void**) &device_k, kernelArrayLength*sizeof(float));
    cudaMalloc((void**) &device_y, outputArrayLength*sizeof(float));

    cudaMemcpy(device_x, host_x, inputArrayLength*sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(device_k, host_k, kernelArrayLength*sizeof(float), cudaMemcpyHostToDevice);
    ...
    // Version 2
    // dim3 gridDim(ceil(1.0*W_out/TILE_WIDTH), ceil(1.0*H_out/TILE_WIDTH), M);
    // dim3 gridDim((W_out-1)/TILE_WIDTH + 1, (H_out-1)/TILE_WIDTH + 1, M);
    ...
    cudaMemcpy(host_y, device_y, outputArrayLength*sizeof(float), cudaMemcpyDeviceToHost);
